...
describe('MockResponseBodyStream', () => {
    ...
    it('JSON-encodes falsy non-string bodies', async () => {
        const stream = new MockResponseBodyStream(false, () => {});
        const reader = stream.getReader();
        const {value} = await reader.read();
        assert.strictEquals(new TextDecoder().decode(value), 'false');
    });
...
describe(createMockResponse.name, () => {
    ...
    it('supports Response.text() for an empty body', async () => {
        const response = createMockResponse();
    ...
    it('lets caller-provided content-type replace the default', () => {
        const response = createMockResponse({
            body: {
                hi: 'bye',
            },
