...
export function createMockResponse(params: Readonly<MockResponseParams> = {}): Response {
    ...
    const hasExplicitContentType = !!readHeaderValue(headersToObject(headers), 'content-type')
        .length;
    ...
    return {
        ...
        headers: mergeHeaders(
            body == undefined || hasExplicitContentType
                ? {}
                : {
        ...
        text() {
             ...
             */
            return Promise.resolve(
                body == undefined ? '' : check.isString(body) ? body : JSON.stringify(body),
            );
        },
        json() {
            if (bodyUsed) {
