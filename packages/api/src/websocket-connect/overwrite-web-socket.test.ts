...
describe(verifyWebSocketMessage.name, () => {
    ...
    it('accepts an undefined message when the WebSocket has no message shapes', () => {
        assert.isUndefined(
            verifyWebSocketMessage(noMessagesWebSocket, undefined, WebSocketLocation.OnClient),
        );
    });

    it('rejects falsy defined messages when the WebSocket has no message shapes', () => {
        assert.throws(
            () => verifyWebSocketMessage(noMessagesWebSocket, false, WebSocketLocation.OnClient),
            {
                matchMessage: 'does not expect any message data',
            },
        );
