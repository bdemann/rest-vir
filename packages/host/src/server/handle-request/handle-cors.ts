...
export async function handleCors(
    ...
    const methodDefinition =
        route.isEndpoint && check.isKeyOf(methodForOriginCheck, route.definition.requests)
            ? route.definition.requests[methodForOriginCheck]
            : undefined;
    const requiredRequestHeaderKeys = getObjectTypedKeys(
        methodDefinition?.requiredRequestHeaders || {},
    );
...
function buildOptionsRequestCorsHeaders({
    ...
    return {
        ...
        'Access-Control-Allow-Headers': removeDuplicates([
            'Cookie',
            'Authorization',
            'Content-Type',
            ...(customHeaders || []),
