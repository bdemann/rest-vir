...
const uncheckedRequestDataEndpoint = defineEndpoint({
    path: '/unchecked-request-data',
    requests: {
        [HttpMethod.Post]: {
            responses: {
                [HttpStatus.Ok]: {
...
const requiredHeaderEndpoint = defineEndpoint({
    path: '/required-header',
    requests: {
        [HttpMethod.Post]: {
...
const gapsApi = defineApi({
    ...
    endpoints: [
        ...
        uncheckedRequestDataEndpoint,
        requiredHeaderEndpoint,
        formDataEndpoint,
...
const gapsImplementation = implementApi<undefined>()(gapsApi, {
    ...
    endpoints: [
        ...
        gapsImplementor.implementEndpoint(uncheckedRequestDataEndpoint, {
            [HttpMethod.Post]({requestData}) {
                return {
                    [HttpStatus.Ok]: {
                        responseData: stringify(requestData),
                    },
                };
            },
        }),
        gapsImplementor.implementEndpoint(requiredHeaderEndpoint, {
            [HttpMethod.Post]() {
                return {
                    [HttpStatus.Ok]: {
                        responseData: undefined,
...
describe('coverage-gaps integration', () => {
    it('exercises endpoint and websocket gaps', async () => {
        ...
        try {
            ...
            /**
             * Omitting requestData from the endpoint definition disables request body validation
             * while still passing the parsed body to the implementation.
             */
            const uncheckedRequestDataResponse = await fetch(
                `http://localhost:${port}/unchecked-request-data`,
                {
                    method: HttpMethod.Post,
                    headers: {
                        'content-type': 'application/json',
            ...
            assert.strictEquals(uncheckedRequestDataResponse.status, HttpStatus.Ok);
            assert.strictEquals(
                await uncheckedRequestDataResponse.json(),
                stringify({
                    unchecked: true,
                }),
            );

            const missingRequiredHeaderResponse = await fetch(
                `http://localhost:${port}/required-header`,
                {
                    method: HttpMethod.Post,
                },
            );
            assert.strictEquals(missingRequiredHeaderResponse.status, HttpStatus.BadRequest);

            const invalidRequiredHeaderResponse = await fetch(
                `http://localhost:${port}/required-header`,
                {
                    method: HttpMethod.Post,
                    headers: {
                        'x-required-token': 'invalid',
                    },
                },
            );
            assert.strictEquals(invalidRequiredHeaderResponse.status, HttpStatus.BadRequest);

            const validRequiredHeaderResponse = await fetch(
                `http://localhost:${port}/required-header`,
                {
                    method: HttpMethod.Post,
                    headers: {
                        'X-Required-Token': 'token-abc',
                    },
                },
            );
            assert.strictEquals(validRequiredHeaderResponse.status, HttpStatus.Ok);
